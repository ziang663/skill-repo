...
async def run(args):
    ...
    async with aiohttp.ClientSession(connector=connector(), timeout=timeout, trust_env=False) as session:
        ...
        adaptive = args.scenario == 'sla'
        if adaptive:
            manifest['adaptive_sampling'] = json.loads((ROOT/'adaptive-sampling-policy-20260914.json').read_text())
        ...
        async def send(i, planned):
            return await request(session, bodies[i], length, output, i, planned)

        def on_result(row, rows, elapsed):
            i = row['id']
            save(out/"requests"/f"{i:04}.json", row)
            save(out/"progress.json", {"phase": "formal", "completed": len(rows), "total": n,
                 "success": sum(r["success"] for r in rows), "elapsed_s": elapsed, "utc": utc()})
            if len(rows)%10 == 0 or not row["success"]:
                print(json.dumps({"point": args.name, "completed": len(rows), "n": n,
                      "success": sum(r["success"] for r in rows), "elapsed_s": round(elapsed, 1)}), flush=True)

        rows, duration, sampling = await dispatch(n, args.rate, send, on_result,
            (lambda results: early_decision(results, profile, hit, args.rate)) if adaptive else None)
        save(out/'sampling-decision.json', sampling)
        planned_n = n
        n = sampling['submitted_requests']
    ...
    summary = {"utc": utc(), "status": "PASS" if not reasons else "FAIL", "failure_reasons": reasons,
         ...
         "planned_maximum_requests": planned_n, "adaptive_sampling": sampling,
    ...
    save(out/"progress.json", {"phase": "done", "completed": n, "success": len(good), "planned_maximum": planned_n, "early_stopped": sampling['early_stopped'], "utc": utc()})
